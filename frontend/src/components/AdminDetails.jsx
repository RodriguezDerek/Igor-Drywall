    ...
    return(
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-50">
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-sm shadow-lg w-full max-w-2xl relative animate-fadeIn max-h-[80vh] overflow-y-auto pl-7 pt-5 pr-5 pb-5">
                ...
                {/* Title */}
                <input className="text-lg text-[#252525] font-semibold mb-4 focus:outline-none focus:border-none plus-jakarta-700" name="name" type="text" value={formData.name} onChange={handleChange}></input>
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input name="clientName" type="text" className="plus-jakarta-500 w-full border border-[#DBDBDB] rounded-md px-3 py-2 text-sm text-[#252525] bg-gray-50" value={formData.clientName} onChange={handleChange}/>
                    </div>

                    <div>
                        <p className="text-xs font-medium text-gray-500 mb-1 plus-jakarta-700">Client Phone Number</p>
                        <input name="clientPhoneNumber" type="text" className="plus-jakarta-500 w-full border border-[#DBDBDB] rounded-md px-3 py-2 text-sm text-[#252525] bg-gray-50" value={formData.clientPhoneNumber} onChange={handleChange}/>
                    </div>

                    <div>
                        <p className="text-xs font-medium text-gray-500 mb-1 plus-jakarta-700">Contractor Name</p>
                        <input name="contractorName" type="text" className="plus-jakarta-500 w-full border border-[#DBDBDB] rounded-md px-3 py-2 text-sm text-[#252525] bg-gray-50" value={formData.contractorName} onChange={handleChange} />
                    </div>

                    <div>
                        <p className="text-xs font-medium text-gray-500 mb-1 plus-jakarta-700">Contractor Phone Number</p>
                        <input name="contractorPhoneNumber" type="text" className="plus-jakarta-500 w-full border border-[#DBDBDB] rounded-md px-3 py-2 text-sm text-[#252525] bg-gray-50" value={formData.contractorPhoneNumber} onChange={handleChange} />
                    </div>

                    <div>
                        <p className="text-xs font-medium text-gray-500 mb-1 plus-jakarta-700">Start Date</p>
                        <input name="startDate" type="date" className="plus-jakarta-500 w-full border border-[#DBDBDB] rounded-md px-3 py-2 text-sm text-[#252525] bg-gray-50" value={formData.startDate} onChange={handleChange} />
                    </div>

                    <div>
                        <p className="text-xs font-medium text-gray-500 mb-1 plus-jakarta-700">Project Status</p>
                        <select name="projectStatus" value={formData.projectStatus} onChange={handleChange} className="plus-jakarta-500 w-full border border-[#DBDBDB] rounded-md px-3 py-2 text-sm text-[#252525] bg-gray-50">
                ...
                <div className="mt-4">
                    ...
                    <input name="team" type="text" className="plus-jakarta-500 w-full border border-[#DBDBDB] rounded-md px-3 py-2 text-sm text-[#252525] bg-gray-50" value={formData.team} onChange={handleChange} />
                </div>

                {/* Job Address */}
                <div className="mt-4">
                    <p className="text-xs font-medium text-gray-500 mb-1 plus-jakarta-700">Job Address</p>
                    <input name="address" type="text" className="plus-jakarta-500 w-full border border-[#DBDBDB] rounded-md px-3 py-2 text-sm text-[#252525] bg-gray-50" value={formData.address} onChange={handleChange} />
                </div>

                {/* Job Description */}
                <div className="mt-4">
                    <p className="text-xs font-medium text-gray-500 mb-1 plus-jakarta-700">Job Description</p>
                    <input name="description" type="text" className="plus-jakarta-500 w-full border border-[#DBDBDB] rounded-md px-3 py-2 text-sm text-[#252525] bg-gray-50" value={formData.description} onChange={handleChange} />
                </div>

                <button className="mt-8 min-w-full h-9 rounded-md custom-red-color-background text-white cursor-pointer plus-jakarta-700">Save</button>

                <hr className="mt-8 border-0 h-px bg-[#DBDBDB]" />

                {/* File Title */}
                <h1 className="text-lg text-[#252525] font-semibold mt-4 plus-jakarta-700">Attached Job Files</h1>
                ...
                <div className="flex justify-center pt-4">
                    <div className="space-y-4 w-full max-w-2xl">
                                ...
                                files.map((file, index) => (
                                    <div key={index} className="flex items-center justify-between border border-gray-300 rounded-lg px-4 py-2 w-full">
                                        ...
                                        <div className="flex space-x-2">
                                            <button onClick={() => downloadFile(projectDetails.id, file)} className="border border-gray-400 rounded px-3 py-1 text-xs hover:bg-gray-100 cursor-pointer plus-jakarta-700">Download</button>
                                            <button onClick={() => removeFile(projectDetails.id, file)} className="bg-red-800 text-white rounded px-3 py-1 text-xs hover:bg-red-900 cursor-pointer plus-jakarta-700">Delete</button>
                                        </div>
                                    </div>
                                ))
                            ) : (
                                <p className="text-gray-500 text-sm text-center plus-jakarta-500">No Files found for {projectDetails.name}</p>
                            )}                            
                        </div>
                            ...
                            <div className="flex items-center space-x-4">
                                <div {...getRootProps()} className={`flex-1 border-2 border-dashed border-red-800 bg-red-50 rounded-lg p-4 text-center cursor-pointer hover:bg-red-100 h-14 flex items-center justify-center ${isDragActive ? "bg-red-100" : ""}`}>
                                    ...
                                    <div className="flex items-center justify-center space-x-2 text-red-800 font-medium">
                                        ...
                                        <span className="text-sm plus-jakarta-700">{isDragActive ? "Drop the file here..." : "Click or drag file to upload"}</span>
                                    </div>
                                </div>

                                {image && (
                                    <button onClick={() => addFile(projectDetails.id)} className="plus-jakarta-700 bg-red-800 text-white px-4 py-1 rounded hover:bg-red-900">Send</button>
                                )}
                            </div>
                        ...
                        {/* Supported Formats */}
                        <p className="text-xs text-gray-500 italic text-center plus-jakarta-500">Supported formats: <span className="not-italic plus-jakarta-500">PDF, JPG, PNG, XLSX, DWG</span></p>
